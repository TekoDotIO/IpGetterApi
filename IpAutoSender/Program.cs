...
namespace RemoteAddressUploader
{
    public class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("IpAutoUploader v.1.0.0.0\nBy EachOther Tech");
            bool restart = false;
            ...
            else
            {
                ...
                Console.WriteLine(Server + "/api/ipgetter/sendip?password=" + Passwd);
                string ip = Get(Server + "/api/ipgetter/sendip?password=" + Passwd);
                if (ip == "403")
                {
                    Console.WriteLine("Failed");
                    return;
                }
                Console.WriteLine(ip + " is sent to the server");
